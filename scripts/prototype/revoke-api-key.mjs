// Revoke a Frege prototype API key by key prefix.
//
// Required env:
//   DATABASE_URL
//
...
if (!keyPrefix) {
  console.error("Usage: node --env-file=.env.local scripts/prototype/revoke-api-key.mjs <key-prefix>");
  process.exit(1);
}
...
if (!key) {
  console.error(`No API key found for prefix=${keyPrefix}.`);
  process.exit(1);
}

console.log("Revoked prototype API key.");
