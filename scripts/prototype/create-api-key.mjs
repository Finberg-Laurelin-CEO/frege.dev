...
//
// Usage:
//   node --env-file=.env.local scripts/prototype/create-api-key.mjs <org-slug> <role-slug> [key-name] [owner-email]
//
// Example:
...
const ownerEmail = process.argv[5]?.trim().toLowerCase();

if (!orgSlug || !roleSlug) {
  console.error("Usage: node --env-file=.env.local scripts/prototype/create-api-key.mjs <org-slug> <role-slug> [key-name] [owner-email]");
  process.exit(1);
}
...
const [owner] = ownerEmail
  ? await sql`
      ...
      where organization_memberships.org_id = ${role.org_id}
        and users.email = ${ownerEmail}
  ...
  : await sql`
      ...
      where organization_memberships.org_id = ${role.org_id}
        and organization_memberships.status = 'active'
      order by (organization_memberships.role = 'owner') desc, organization_memberships.created_at asc
      limit 1
    `;
...
if (!owner) {
  console.error(
    ownerEmail
      ? `No active org member found for owner email=${ownerEmail}.`
      : "No active org member found to own this key. Bootstrap a user or pass an owner email.",
  );
  process.exit(1);
}
...
const [row] = await sql`
  ...
  values (${role.org_id}, ${role.role_id}, ${owner.id}, ${keyName}, ${key.keyPrefix}, ${key.keyHash})
